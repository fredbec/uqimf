...
ovr_assessment_plot <- function(wis_scoredat, cvg_scoredat, point_scores, crps_scores, trgt){
  ...
  wis_plot <- function(decompdat, error_m, maxval){
    ...
    if(error_m == "directional"){
      plot_name <- "WIS - Directional Errors"
    } else {
      plot_name <- "WIS - Absolute Errors"
    }
  ...
  horizon_path_plot <- function(score_data,
    ...
    path_plot <- ggplot(score_data, 
      ...
      scale_color_met_d("Hokusai1", breaks = c("IMF", "bvar", "ar", "bvar_qu"),
  ...
  crpsplot_rw <- horizon_path_plot(crps_scores_rw |>.d(, error_method := "directional"), "score", "CRPS by Sample", title = "CRPS - Rolling Window") +
...
```{r gdpovr_ew, echo = FALSE, warning = TRUE, fig.dim=c(12,12)}

ovr_assessment_plot(wis_scores, scores_cvgshort, point_scores, all_scores, "ngdp_rpch")
```
...
```{r overall_assessment, fig.dim=c(12,12), echo = FALSE, warning = FALSE}
...
ovr_assessment_plot <- function(wis_scoredat, cvg_scoredat, point_scores, crps_scores, trgt){
  ...
  crpsplot_rw <- horizon_path_plot(crps_scores_rw |>.d(, error_method := "directional"), "score", "CRPS by Sample", title = "CRPS - expanding window") +
    scale_y_continuous(limits = c(0.07,1.05)) 
...
```{r gdpovr, echo = FALSE, warning = TRUE, fig.dim=c(12,12)}
