...
dat_fc <- read.csv(here("data", "weodat.csv")) %>% filter(horizon == 0) %>%
  transmute(country, target, target_year, prediction, tv_0.5, tv_1)

dat_sc <- read.csv(here("data", "weodat.csv")) %>% filter(horizon == 0.5) %>%
  transmute(country, target, target_year, prediction, tv_0.5, tv_1)

dat_fn <- read.csv(here("data", "weodat.csv")) %>% filter(horizon == 1) %>%
  transmute(country, target, target_year, prediction, tv_0.5, tv_1)


dat_sn <- read.csv(here("data", "weodat.csv")) %>% filter(horizon == 1.5) %>%
  transmute(country, target, target_year, prediction, tv_0.5, tv_1)
  ...
  for (forecast_season in c("S", "F")){
    ...
    for (vv in vnms){
      ...
      for (yy in yrs_fcst){
        ...
        tmp7 <- tmp1 %>%
          filter(target_year <= (yy)) %>%
          arrange(target_year) %>% pull(prediction)
        # check data length
        stopifnot(length(tmp2) == (yy-2-1990+1))
        #stopifnot(length(tmp7) == (yy-2-1990+1))
