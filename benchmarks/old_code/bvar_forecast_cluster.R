...
for (yy in yrs_fcst){
  ...
  if (check1 & check2){
    ...
    # run model
    fit <- bvar.sv.tvp(dat, nf = 7, nrep = 2e4)
    ...
    if (fcst_season == "S"){
      ...
      aux2 <- apply(fit$fc.mdraws, 1:2, mean)[,1:3]
      # combine and compute weighted sum
      aux3 <- rbind(aux1, t(aux2))
      fcst_current <- c(sum(weights*aux3[,1]), 
                        sum(weights*aux3[,2]))
      # next year forecast
      # get relevant forecasts
      aux4 <- apply(fit$fc.mdraws, 1:2, mean)[,1:7] %>% t
      ...
      aux1 <- tail(dat, 6)
      # get relevant forecasts
      aux2 <- apply(fit$fc.mdraws, 1:2, mean)[,1]
      # combine and compute weighted sum
      aux3 <- rbind(aux1, t(aux2))
      fcst_current <- c(sum(weights*aux3[,1]),
                        sum(weights*aux3[,2]))
      ...
      aux4 <- tail(dat, 2)
      # get relevant forecasts
      aux5 <- apply(fit$fc.mdraws, 1:2, mean)[,1:5]
      aux6 <- rbind(aux4, t(aux5))
...
# save forecasts
write.csv(df_all, 
          file = paste0("bvar_imf/fcst_", cc, "_", fcst_season, ".csv"), 
          row.names = FALSE)