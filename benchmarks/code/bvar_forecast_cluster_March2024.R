...
all_settings <- data.frame(country = rep(g7_oecd, each = 2), 
                           season = rep(c("S", "F"), 7)) %>%
  (function(x) rbind(data.frame(x, with_ciss = TRUE), 
                     data.frame(x, with_ciss = FALSE))) %>%
...
ciss_addon <- if_else(with_ciss, "_with_ciss_", "")

# save forecasts
write.csv(df_all, 
          file = paste0("bvar_imf_2024/fcst_quantiles_", cc, 
                        "_", fcst_season, ciss_addon, ".csv"), 
          row.names = FALSE)