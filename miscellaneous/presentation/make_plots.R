...
wis_scores <- fread(here("scores", "ci_scores_avgcnt_pava.csv")) |>
  data.table::copy() |>
...
####Coverage Data
scores_cvgshort <- fread(here("scores", "ci_scores_avgcnt_pava.csv")) |>
...
cpi_plot <- wis_plot_new(wis_scores, "pcpi_pch")

gdp_plot <- wis_plot_new(wis_scores, "ngdp_rpch")
#cpi_plot <- ovr_assessment_plot(wis_scores, "pcpi_pch")
#gdp_plot <- ovr_assessment_plot(wis_scores, "ngdp_rpch")



pdf(file = here("presentation", "figures", "wis_cpigdp_new.pdf"), width = 9.5, height = 4)
ovr_plot <-
  (cpi_plot | gdp_plot) +
