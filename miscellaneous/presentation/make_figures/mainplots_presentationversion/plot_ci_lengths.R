...
imf_fc <- fread(here("quantile_forecasts", paste0("quantile_forecasts_ho", ".csv"))) |>
  .d(source == "IMF") |>
  ...
  .d(, cil5 := q0.75-q0.25) |>
  .d(, cil9 := q0.9 - q0.1) |>
  .d(, meancil5 := round(mean(cil5),10), by = c("horizon", "target")) |>
  .d(, meancil9 := round(mean(cil9),10), by = c("horizon", "target")) |>
  ...
  .d(, target := factor(target,
                        ...
                        labels = c("I: Inflation", "G: GDP Growth"),
                        ordered = "TRUE") )
...
legend.box.margin=margin(-15,-10,15,-10)) +
  annotate("label", x = xl, y = ypos, label = "I", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xr, y = ypos, label = "G", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xl+1, y = ypos, label = "I", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xr+1, y = ypos, label = "G", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xl+2, y = ypos, label = "I", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xr+2, y = ypos, label = "G", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xl+3, y = ypos, label = "I", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  annotate("label", x = xr+3, y = ypos, label = "G", size = 3, fill = NA, label.size = 0, color = "black",family = "serif") +
  #annotate("label", x = 1.18, y = -2.18, label = "G", size = 3, fill = "white", color = "black",family = "serif") +
  scale_alpha_manual(name = "",
                     breaks = c("50% Interval", "80% Interval"),
                     values = c("50% Interval" = 0.8, "80% Interval" = 0.4) )
ggsave(here("..", "uqimf-manuscript", "figures", paste0(global_file_prefix, "cilength_byhorizon_ho.pdf")), cilengthplot, width = 4.75, height = 3.25)
