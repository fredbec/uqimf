...
ciscores <- fread(here("scores", paste0("ci_scores_avgcnt_ho.csv"))) |>
  .d(, c("model", "target", "horizon", "interval_score_50", "interval_score_80")) |>
  .d(, target := ifelse(target == "ngdp_rpch", "GDP", "CPI")) |>
  .d(model %in% c("IMF", "ar", "bvar_const")) |>
  .d(, model := gsub("_", "-", model))

ciscores2 <- fread(here("scores", paste0("bvar_ci_scores_avgcnt_ho.csv"))) |>
  .d(, c("model", "target", "horizon", "interval_score_50", "interval_score_80")) |>
  .d(, target := ifelse(target == "ngdp_rpch", "GDP", "CPI")) |>
  .d(model == "bvar_const") |>
  .d(, model := gsub("_", "-", model)) |>
...
create_latex_table2 <- function(dat, mrow){
  ...
  round_cols_newer <- c("$\\text{IS}_{50}^{1)}$", "$\\text{IS}_{80}^{1)}$")#,
  ...
  singletab <- lapply(dat, function(dt){
    ...
    round_cols <- c("interval_score_50", "interval_score_80")#, "dev_50", "dev_80")
...
writeLines(table_current, (here("manuscript_plots", "revision", "results", "base_is5080scores.tex")))
writeLines(table_current, (here("..", "uqimf-manuscript", "tables", "base_is5080scores.tex")))
