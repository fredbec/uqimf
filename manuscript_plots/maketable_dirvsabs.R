...
scores_cvgshort_abs <- fread(here("scores", paste0("ci_scores_avgcnt", "_pava", ".csv"))) |>
  ...
  .d(, interval_score := round(interval_score, 3)) |>
  .d(, coverage_80 := round(coverage_80, 2))|>
  .d(, coverage_50 := round(coverage_50, 2))



scores_cvgshort_dir <- fread(here("scores", paste0("ci_scores_avgcnt", ".csv"))) |>
