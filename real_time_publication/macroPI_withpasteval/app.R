    ...
    column(12,
           ...
           tabsetPanel(type = "tabs",
                       ...
                       tabPanel(paste0(release_title, " Forecasts - GDP Growth"), plotOutput(outputId = "allcplot_gdp",
                                                                                             height = 1000, width = 1000)),
                       tabPanel(paste0(release_title, " Forecasts - Inflation - Evaluation"),radioButtons("horizon_infl", "Forecast Horizon:",
                                                                                                          c("Fall, Current" = 0,
                                                                                                            "Spring, Current" = 0.5,
                                                                                                            "Fall, Next" = 1,
                                                                                                            "Fall, Next" = 1.5)),
                                plotOutput(outputId = "allcplot_inflation_eval",
                                                                                             height = 1000, width = 1000)),
                       tabPanel(paste0(release_title, " Forecasts - GDP Growth - Evaluation"),radioButtons("horizon_gdp", "Forecast Horizon:",
                                                                                                          c("Fall, Current" = 0,
                                                                                                            "Spring, Current" = 0.5,
                                                                                                            "Fall, Next" = 1,
                                                                                                            "Fall, Next" = 1.5)),
                                plotOutput(outputId = "allcplot_gdp_eval",
                                           height = 1000, width = 1000))
           )
    )
  )
...
server <- function(input, output) {
  ...
  infl_eval <- reactive({
    ...
    qufcs_eval <- data.table::fread(here("quantile_forecasts", "quantile_forecasts.csv"))|>
      ...
      .d(horizon == horizon_eval) |>
      .d(, forecast_year := ceiling(target_year - horizon)) |>
      .d(, forecast_season := ifelse(round(horizon_eval) == horizon_eval, "F", "S")) |>
      .d(, c("country", "target", "forecast_year", "forecast_season", "target_year", "quantile", "prediction")) |>
  ...
  output$allcplot_inflation_eval <- renderPlot(
  ...
  gdp_eval <- reactive({
    ylimmax <- 9.5
    trgt <- "gdp_growth"
    ylabel <- "Real GDP growth rate (in %)"
    textlabel <- "GDP Growth"
    ...
    qufcs_eval <- data.table::fread(here("quantile_forecasts", "quantile_forecasts.csv"))|>
      ...
      .d(, forecast_season := ifelse(round(horizon_eval) == horizon_eval, "F", "S")) |>
      .d(, c("country", "target", "forecast_year", "forecast_season", "target_year", "quantile", "prediction")) |>
      .d(, target := ifelse(target == "pcpi_pch", "inflation", "gdp_growth"))
  ...
  output$allcplot_gdp_eval <- renderPlot(
